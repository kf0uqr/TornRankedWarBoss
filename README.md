...
`/current_war` posts an enemy-roster status board (level, estimated stats, status, last action, position, wall/revivable flags) for whichever ranked war is currently active, then edits that same message in place every 5 minutes for as long as the war runs. The board stops updating on its own once the war ends; run `/current_war` again for the next one.
