...
- **Pooled API keys** — Torn caps each key at 100 requests/minute. Add more keys in Settings (e.g. from other faction members) and the app round-robins requests across all of them, multiplying your effective throughput. A small fixed slice of the pool (`db.BOT_RESERVED_KEY_COUNT`, default 2 keys) is reserved exclusively for the Discord bot's own background polling, so it can never crowd out an interactive web app user waiting on a page load - everyone using the web app draws from the rest of the pool.
