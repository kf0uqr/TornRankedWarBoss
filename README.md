...
- **Discord bot** — read-only slash commands (`/wars`, `/paysheet`, `/stats`, `/career`, `/armory`, `/current_war`) so leadership can check things from Discord without needing access to the machine running the app, plus `/add_api_key` (open to any faction member, not just leadership) to submit their own Torn API key into the app's pool without needing app access at all, and `/new_giveaway`/`/del_giveaway` (also open to everyone) to run and cancel button-entry giveaways with randomly-chosen winners. `/current_war` posts live status boards for the active ranked war - enemy roster, your own, and an enemy activity heatmap - and keeps editing them in place every few minutes. No port forwarding or router changes needed — see below.
...
`/del_giveaway giveaway:<pick from the list>` cancels a still-running giveaway before its timer ends - the parameter autocompletes with every active giveaway's name and prize as you type. Only the giveaway's own creator or leadership can actually cancel it (being on the allowed list alone isn't enough to cancel someone else's); the original post gets edited to show it was cancelled and its Enter button is removed, but the record stays in the database with a `cancelled` status distinct from a completed draw.
...
5. Add each faction member's Discord **User ID** (not username) under "Allowed Discord Users" — turn on Developer Mode in Discord (Settings → Advanced) to right-click someone and "Copy User ID". Only these accounts can use the bot's commands at all, and check **Leadership** for anyone who should have access to leadership-only commands: `/wars`, `/paysheet`, `/stats`, `/career`, `/armory`, and `/current_war` all require it. `/new_giveaway` and `/del_giveaway` don't - anyone on the allowed-users list can use them, leadership or not (though `/del_giveaway` still only lets you cancel your own giveaway unless you're leadership). `/add_api_key` doesn't check the list at all - it's open to any Discord user (see below for why).
